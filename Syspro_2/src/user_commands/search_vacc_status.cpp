...
void search_vaccination_status(int buffer_size, int num_of_monitors, std::string *input_array, monitor_db **monitor_array){
    ...
    for(int i=0; i<num_of_monitors; i++){                                     //Communicate with the monitors to find the one that has the ID.
        ...
        send_message(buffer_size,monitor_array[i]->get_pipe_write(),monitor_array[i]->get_pipe_read(),input_array[1]);
    ...
    if(!monitor_with_id){                                                     //If the ID is not found, return error.
        ...
        receive_message(buffer_size,monitor_with_id->get_pipe_write(),monitor_with_id->get_pipe_read(),citizen_details);
        receive_message(buffer_size,monitor_with_id->get_pipe_write(),monitor_with_id->get_pipe_read(),citizen_viruses);
...
void search_vaccination_status_monitor(int buffer_size, int num_of_vrs, int pipe_write, int pipe_read, skip_list ***vaccine_skip_l, skip_list *information_check){
    ...
    receive_message(buffer_size,pipe_write,pipe_read,ID);   //Receive citizen ID.

    block *ctzn = information_check->search(std::stoi(ID)); //Search for citizen ID.

    if(ctzn){//Inform whether or not citizen was found
        send_message(buffer_size,pipe_write,pipe_read,"YES");
     }
    else{
    ...
    //If the citizen was found, send his details, as well as the details of the viruses he is recorded for.
    ...
    vaccine_status(std::stoi(ID),num_of_vrs,vaccine_skip_l,citizen_viruses);

    send_message(buffer_size,pipe_write,pipe_read,citizen_viruses);
}

...
void travel_stats(int buffer_size, int num_of_words, int num_of_monitors, std::string *input_array, monitor_db **monitor_array){
    ...
    if(num_of_words == 4){                                                               //If the global statistics are required.
        ...
        for(int i=0; i<num_of_monitors; i++){                                            //Send the message to all the monitors.
            send_message(buffer_size,monitor_array[i]->get_pipe_write(),monitor_array[i]->get_pipe_read(),command);
            send_message(buffer_size,monitor_array[i]->get_pipe_write(),monitor_array[i]->get_pipe_read(),message);
            
            for(int j=0; j<monitor_array[i]->get_num_of_countries(); j++){               //Receive each Monitor's statistics.
                receive_message(buffer_size,monitor_array[i]->get_pipe_write(),monitor_array[i]->get_pipe_read(),response);

                response_array = split_to_words(response,dummy_num);

                accepted_req += std::stoi(response_array[0]);                            //Add all the results together.
                rejected_req += std::stoi(response_array[1]);
        ...
        send_message   (buffer_size,monitor_with_country->get_pipe_write(),monitor_with_country->get_pipe_read(),command);
        send_message   (buffer_size,monitor_with_country->get_pipe_write(),monitor_with_country->get_pipe_read(),message);
        receive_message(buffer_size,monitor_with_country->get_pipe_write(),monitor_with_country->get_pipe_read(),response);
        ...
        std::cout << "      Total    Requests: " << (std::stoi(response_array[0]) + std::stoi(response_array[1])) << "." << std::endl;
...
void travel_stats_monitor(int buffer_size, int num_of_folders, int pipe_write, int pipe_read, country_records **monitor_countries){
    ...
    receive_message(buffer_size,pipe_write,pipe_read,message);      //Receive the message from travelMonitor.

...
class monitor_db{ //A class used by travelMonitor, to store needed information about the Monitors.
    private:
        int            pipe_read;                                                   //The file descriptor of the pipe that travelMonitor will be reading.
        int            pipe_write;                                                  //The file descriptor of the pipe that travelMonitor will be writting.
        ...
        pid_t          monitor_pid;                                                 //The Monitor's pid.
    ...
    public:
        monitor_db(int monitor_num, long bloom_size, pid_t ID, std::string input);
        ~monitor_db();
        ...
        inline int  get_pipe_read(){return this->pipe_read;};                       //Get the pipe that the travelMonitor reads from (and the Monitor writes to).
        inline int  get_pipe_write(){return this->pipe_write;};                     //Get the pipe that the travelMonitor writes to (and the Monitor reads from).
        ...
        inline long get_monitor_pid(){return long(this->monitor_pid);};             //Get the monitor pid.

        inline void update_pid(pid_t new_pid){this->monitor_pid = new_pid;};        //Update the monitor pid.
        inline void set_pipe_read(int pipe_read){this->pipe_read = pipe_read;};     //Set the monitor pipe.
        inline void set_pipe_write(int pipe_write){this->pipe_write = pipe_write;}; //Set the monitor pipe.

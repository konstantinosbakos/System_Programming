...
char *pipe_name(int i, int type){ //Creates the name of the pipes.
    int   num_iter   = 0;
    int   num_length = i;
    
    while(num_length){
        num_length = (num_length - num_length%10)/10;
        num_iter++;
     }
    
    char *name = new char[14 + num_iter];
    ...
    num_length = num_iter;
    while(num_iter){
        name[10 + num_iter] = '0' + i%10;
        i = (i - i%10)/10;
        num_iter--;
     }
    
    name[11 + num_length] = '_';
    name[12 + num_length] = '0' + type;
    name[13 + num_length] = '\0';

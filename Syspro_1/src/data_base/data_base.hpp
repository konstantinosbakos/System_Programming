...
class data_base{
    ...
    public:
        data_base(std::string file_name, skip_list *information_check);
        ~data_base();

        void remove_citizen(citizen *ctzn);
        void add_citizen(std::string data, skip_list *information_check); //Add citizen to list.

        citizen *add_citizen(int num_of_words, std::string *Command_Array, skip_list *information_check, int add_citizen_flag, std::string date=""); //Add citizen to list and return the respective pointer.

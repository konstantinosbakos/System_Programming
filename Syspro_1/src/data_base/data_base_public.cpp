...
data_base::data_base(std::string file_name, skip_list *information_check){ //Initialize data base.
    this->vrs_num     = 0;
    this->nam_head    = NULL;
    this->vrs_head    = NULL;
    this->ctzn_head   = NULL;
    ...
    while(getline(file,str)){
        this->add_citizen(str,information_check); //Add citizens in the data base from the imput file.
     }
}
...
citizen *data_base::add_citizen(int num_of_words, std::string *Command_Array, skip_list *information_check, int add_citizen_flag, std::string date){ //Add citizen from user input.
    citizen     *new_citizen = new citizen;
    std::string *broken_date;

    if(is_citizen_vaccinated(num_of_words,date)){ //If one of the input cases is found, then citizen is vaccinated.

        if((date == "") && !(check_date_format(Command_Array[8]))){
            std::cout << "Error: Please check the date format." << std::endl;
            delete new_citizen;
            return NULL;
        }

        if((date == "") && !(is_valid_date(Command_Array[8]))){
            std::cout << "Error: Please enter a valid date." << std::endl;
            delete new_citizen;
            return NULL;
         }

        new_citizen->set_ID(std::stoi(Command_Array[1]));
        new_citizen->set_first_name(check_name(Command_Array[2]));
        new_citizen->set_surname(check_surname(Command_Array[3]));
        new_citizen->set_country(check_country(Command_Array[4]));
        new_citizen->set_age(int(std::stoi(Command_Array[5])));
        new_citizen->set_virus(check_virus(Command_Array[6]));
        new_citizen->set_is_vacc(VACCINATED);

        if(date != ""){                           //Check if specific date is given.
            broken_date = break_date(date);
         }
        else{
            broken_date = break_date(Command_Array[8]);       
         }

...
int travel_request_send(int socket_buffer_size, int num_of_monitors, std::string *input_array, monitor_db **monitor_array){
    ...
    if(countryFrom_monitor->search_bloom_filter(input_array[1],input_array[5]) != 0){ //Search if ID exists.
        ...
        //If it does, write the information to the respective Monitor.
        send_message(socket_buffer_size,countryFrom_monitor->get_socket_write(),countryFrom_monitor->get_socket_read(),command);
        send_message(socket_buffer_size,countryFrom_monitor->get_socket_write(),countryFrom_monitor->get_socket_read(),input_array[1]);
        send_message(socket_buffer_size,countryFrom_monitor->get_socket_write(),countryFrom_monitor->get_socket_read(),input_array[5]);

        std::string response = "";
        //Receive the response.
        receive_message(socket_buffer_size,countryFrom_monitor->get_socket_write(),countryFrom_monitor->get_socket_read(),response);

        //According to the response, print the right output.
        if(response == "NO"){
            std::cout << "~> Request Rejected: You are not vaccinated." << std::endl;
            send_to_countryTo(socket_buffer_size,input_array[2],input_array[4],input_array[5],countryTo_monitor,REJECTED_REQ);
            ...
            if(compare_dates(response_array[1],OLDEST_DATE,input_array[2])){
                ...
                if((input_date[2] - vaccine_date[2]) >= 2){
                    ...
                    send_to_countryTo(socket_buffer_size,input_array[2],input_array[4],input_array[5],countryTo_monitor,ACCEPTED_REQ);
...
void add_to_destination(int socket_buffer_size, int num_of_folders, int socket_write, int socket_read, virus *vrs_list, country_records **monitor_countries){
    ...
    receive_message(socket_buffer_size,socket_write,socket_read,message);//
    receive_message(socket_buffer_size,socket_write,socket_read,date);
...
void travel_request_receive(int socket_buffer_size, int socket_write, int socket_read, skip_list ***vaccine_skip_l, skip_list *information_check, data_base *citizen_info){
    ...
    receive_message(socket_buffer_size,socket_write,socket_read,ID);
    receive_message(socket_buffer_size,socket_write,socket_read,str_virus);

    if(!is_virus(str_virus,citizen_info->get_vrs_ptr(),true)){                //Search for the virus.
        send_message(socket_buffer_size,socket_write,socket_read,"NO VIRUS"); //If it does not exist, inform travelMonitor and return.
        return;
     }

    skip_list *vac_skip_l = get_skip_list(citizen_info->get_vrs_num(),str_virus,vaccine_skip_l,VACCINATED); //Search for citizen.
    block     *ctzn_vac   = vac_skip_l->search(to_int(ID));

    if(ctzn_vac){                                                             //If citizen is vaccinated, return the vaccination date.
        send_message(socket_buffer_size,socket_write,socket_read,("YES " + ctzn_vac->get_citizen()->get_date()));
     }
    else{
